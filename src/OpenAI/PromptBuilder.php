...
class PromptBuilder
{
    ...
    public function __construct(private Brain $brain, public ?string $api_key = null, public ?int $history_tokens = null)
    {
        ...
        if (!$api_key) throw new FatalException("API key not found in configuration file at $config_file", true);
        if (!$this->validate_api_key($api_key)) throw new FatalException("Invalid API key", true);

        if (!$history_tokens) $history_tokens = $this->prompt["history_tokens"] ?? null;
        unset($this->prompt["history_tokens"]);
        if (!$history_tokens) throw new FatalException("history_tokens not found in configuration file at $config_file", true);
        if (!is_int($history_tokens)) throw new FatalException("Invalid history_tokens in $config_file", true);
